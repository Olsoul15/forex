...
from forex_ai.data.storage.auto_trading_repository import AutoTradingRepository, AutoTradingPreferences
...
auto_trading_repository = AutoTradingRepository()
...
async def get_auto_trading_preferences(
    ...
    try:
        ...
        preferences = await auto_trading_repository.get_preferences(userId)
        
        if not preferences:
            # Return default preferences
            preferences_dict = AutoTradingPreference().model_dump()
        else:
        ...
        return AutoTradingPreferencesResponse(
            userId=userId,
            preferences=AutoTradingPreference(**preferences_dict),
...
async def update_auto_trading_preferences(
    ...
    try:
        ...
        logger.info(f"Updating preferences for user {request.userId}: {request.preferences.model_dump()}")
        
        updated_preferences = await auto_trading_repository.update_preferences(request.userId, request.preferences.model_dump())
        
        if not updated_preferences:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Failed to update auto-trading preferences",
            )
...
async def enable_auto_trading(
    ...
    account = await account_repository.get_account_by_id(request.accountId)
    if not account or account.user_id != request.userId:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    try:
        ...
        preferences = await auto_trading_repository.get_preferences(request.userId)
        
        if not preferences:
            # Create default preferences
            preferences_data = AutoTradingPreference().model_dump()
            preferences_data["user_id"] = request.userId
            preferences_data["enabled"] = True
            
            updated_preferences = await auto_trading_repository.create(preferences_data)
        else:
            ...
            preferences_dict["enabled"] = True
            updated_preferences = await auto_trading_repository.update(preferences.id, preferences_dict)
...
async def disable_auto_trading(
    ...
    try:
        ...
        if not preferences:
            ...
            preferences_data = AutoTradingPreference().model_dump()
            preferences_data["user_id"] = request.userId
            preferences_data["enabled"] = False
            ...
            preferences_dict["enabled"] = False
            updated_preferences = await auto_trading_repository.update(preferences.id, preferences_dict)
...
async def get_auto_trading_stats(
    ...
    try:
        # Get auto-trading stats using the repository
        stats = await auto_trading_repository.get_statistics(userId)
...
async def get_recent_auto_trades(
    ...
    """
    Get recent auto-trading trades.

    Returns a list of recent auto-trading trades for the specified user.
    """
    logger.info(f"Processing auto-trading trades request for user {userId}")
    ...
    try:
        # For now, we'll use mock data since we don't have a trades repository yet
        now = datetime.now()
        trades = [
            {
                "id": f"trade-{i}",
                "account_id": accountId or "demo-account",
                "instrument": ["EUR_USD", "GBP_USD", "USD_JPY", "AUD_USD"][i % 4],
                "direction": "buy" if i % 2 == 0 else "sell",
                "size": 0.1 * (i + 1),
                "entry_price": 1.0950 + (i * 0.0010),
                "exit_price": 1.0970 + (i * 0.0010) if i % 3 != 0 else None,
                "profit_loss": 20.0 * (i + 1) if i % 3 != 0 else None,
                ...
                "exit_time": now - timedelta(hours=12 - i) if i % 3 != 0 else None
